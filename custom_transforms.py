...
class Superimposed:
    ...
    def align_timestamps(self):
        # Start
        # if self.frg['t'].min() > self.bkg['t'].min():
        min_t = self.frg['t'].min()
        self.bkg = self.bkg[self.bkg[:]['t'] >= min_t]
    ...
    def downsample_bkg(self):
        if (self.bkg.size > 3000):
            n_events_to_keep = 1000
            p = 1 - n_events_to_keep / self.bkg.size # equivalent to keeping n events - that is (total events - n) / total events
            transform = tonic.transforms.DropEvent(p=p)
            self.bkg = transform(self.bkg)
...
def add_uniform_noise(events, sensor_size, n_noise_events):
    transform = tonic.transforms.UniformNoise(sensor_size=sensor_size, n_noise_events=n_noise_events)
...
def generate_uniform_noise_dataset(dataset, sensor_size=(34,34,2), save_path=None):

    new_dataset = []
    for idx, (events, label) in enumerate(dataset):
        print(idx)
        noised_events = add_uniform_noise(events=events, sensor_size=sensor_size, n_noise_events=1500)

        new_entry = (noised_events, label)
